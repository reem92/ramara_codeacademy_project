...
function upVoteComment(url, request)
{
  ...
  if(id && savedComment &&  username && database.users[username]){
...
function createComment(url, request){
  const requestedComment = request.body && request.body.comment;
  const response = {};

if ( requestedComment && requestedComment.body && requestedComment.username && requestedComment.articleId &&
  database.users[requestedComment.username] && database.articles[requestedComment.articleId]) {
    const comment = {
      id: database.nextCommentId++,
      body: requestedComment.body,
      username: requestedComment.username,
      articleId: requestedComment.articleId,
      upvotedBy:[],
      downvotedBy:[],
    };
